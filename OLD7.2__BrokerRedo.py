...
class MovingAverageCrossoverStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if buy_candidates:
            ##call function here to sort buy candidates by a specific criterion (e.g., highest up probability or lowest corolations)
            buy_candidates = self.sort_buy_candidates(buy_candidates)
            #print(f"Buy candidates: {len(buy_candidates)}")
            # Select the best buy candidates within max positions limit
...
###=====================[ DISCRIMATIONNNNN ]====================###
    ...
    def is_buy_signal(self, d):
        ...
        # Combine all conditions to determine if it's a buy signal
        BuySignal = (
...
def main():
    ...
    if args.BrokerTest > 0:
        ...
        for file_name in tqdm([f for f in file_names if f.endswith('.parquet') and f.split('.')[0].lower() in tickers], desc="Loading Ticker Files"):
    ...
    print(colorize_output(PercentPerTrade, 'Gain Per Trade %:', 2.0, 0.5))
    print(colorize_output(PercentWithoutLargestWinningTradePerTrade, 'Gain without Largest:', 1.5, 0.75))
    print(colorize_output(percentage_gain, 'Percentage Gain:', 100.0, 10.0))
